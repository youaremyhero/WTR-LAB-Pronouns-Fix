...
// @version      4.9.5a
// @description  WTR-LAB Pronouns Fix — stable v4.9.4 + safe UI modes (Add / Export) + ? help + mobile long-press Add Character (Male/Female only). No term-memory clearing.
...
(() => {
  ...
  const UI_KEY_MODE = "wtrpf_ui_mode_v1"; // "none" | "add" | "export"
  const UI_KEY_HELP = "wtrpf_ui_help_v1"; // "0" | "1"
  ...
  function makeUI() {
    ...
    function getMode() { return localStorage.getItem(UI_KEY_MODE) || "none"; }
    function setMode(m) { localStorage.setItem(UI_KEY_MODE, m || "none"); }

    function getHelpOpen() { return localStorage.getItem(UI_KEY_HELP) === "1"; }
    function setHelpOpen(v) { localStorage.setItem(UI_KEY_HELP, v ? "1" : "0"); }
    ...
    // --- Help / onboarding (hidden unless ? is opened)
    const helpWrap = document.createElement("div");
    helpWrap.style.cssText = `display:none; margin-top:10px; opacity:.92;`;
    ...
    helpCard.textContent =
      "Quick help:\n" +
      "• PronounsFix auto-corrects pronouns per chapter.\n" +
      "• Mobile: long-press a WTR term (blue/identified span) to Add Character.\n" +
      "• Use Add New Character to manage Draft; Export Tools to copy Draft / Term Memory.";

    helpWrap.appendChild(helpCard);

    // --- Mode row (Option B)
    const modeRow = document.createElement("div");
    modeRow.style.cssText = `display:flex; gap:8px; margin-top:10px; flex-wrap: wrap;`;

    const modeAddBtn = document.createElement("button");
    modeAddBtn.type = "button";
    modeAddBtn.textContent = "Add New Character";
    modeAddBtn.style.cssText = `
      flex: 1 1 140px;
      appearance:none; border:0; cursor:pointer;
    ...
    modeExportBtn.type = "button";
    modeExportBtn.textContent = "Export Tools";
    modeExportBtn.style.cssText = `
      flex: 1 1 120px;
      appearance:none; border:0; cursor:pointer;
      padding: 8px 10px; border-radius: 10px;
      background: rgba(255,255,255,0.10); color:#fff;
    ...
    modeRow.appendChild(modeAddBtn);
    modeRow.appendChild(modeExportBtn);

    const divider = document.createElement("div");
    divider.style.cssText = `display:none; height:1px; background: rgba(255,255,255,0.12); margin:10px 0;`;

    // --- Add mode: Draft UI (hidden unless mode=add AND draft exists)
    const addWrap = document.createElement("div");
    addWrap.style.cssText = `display:none;`;
    ...
    addBtnRow.style.cssText = `display:flex; gap:8px; margin-top:8px; flex-wrap: wrap;`;
    ...
    addBtnRow.appendChild(copyBtn);
    addBtnRow.appendChild(clearBtn);

    addWrap.appendChild(draftLabel);
    addWrap.appendChild(draftBox);
    addWrap.appendChild(addBtnRow);
    addWrap.appendChild(draftCountRow);

    // --- Export mode UI
    const exportWrap = document.createElement("div");
    exportWrap.style.cssText = `display:none;`;

    const exportLabel = document.createElement("div");
    exportLabel.textContent = "Export Tools";
    exportLabel.style.cssText = `font-weight:600; opacity:.9; margin-bottom:8px;`;

    const exportBtnRow = document.createElement("div");
    exportBtnRow.style.cssText = `display:flex; gap:8px; flex-wrap: wrap;`;

    const exportDraftBtn = document.createElement("button");
    exportDraftBtn.type = "button";
    exportDraftBtn.textContent = "Export Draft";
    exportDraftBtn.style.cssText = `
      appearance:none; border:0; cursor:pointer;
    ...
    const exportTermMemBtn = document.createElement("button");
    exportTermMemBtn.type = "button";
    exportTermMemBtn.textContent = "Export Term Memory";
    exportTermMemBtn.style.cssText = `
      appearance:none; border:0; cursor:pointer;
      padding: 7px 10px; border-radius: 10px;
      background: rgba(255,255,255,0.10); color:#fff;
      font: 12px/1 system-ui, -apple-system, Segoe UI, Roboto, Arial;
    ...
    exportBtnRow.appendChild(exportDraftBtn);
    exportBtnRow.appendChild(exportTermMemBtn);

    exportWrap.appendChild(exportLabel);
    exportWrap.appendChild(exportBtnRow);
    ...
    function renderMode() {
      ...
      addWrap.style.display = (mode === "add") ? "block" : "none";
      exportWrap.style.display = (mode === "export") ? "block" : "none";

      // button tints
      modeAddBtn.style.background = (mode === "add") ? "rgba(255,255,255,0.18)" : "rgba(255,255,255,0.14)";
      modeExportBtn.style.background = (mode === "export") ? "rgba(255,255,255,0.16)" : "rgba(255,255,255,0.10)";
    }

    function renderHelp() {
      helpWrap.style.display = getHelpOpen() ? "block" : "none";
      helpBtn.style.background = getHelpOpen() ? "rgba(255,255,255,0.16)" : "rgba(255,255,255,0.10)";
    }
    ...
    function showGenderPicker(name, onPick) {
      ...
      t.textContent = `Add Character: ${name}`;
      t.style.cssText = `font-weight:600; margin-bottom:12px;`;
    ...
    helpBtn.onclick = () => {
      setHelpOpen(!getHelpOpen());
      renderHelp();
    };

    modeAddBtn.onclick = () => {
      const m = getMode();
      setMode(m === "add" ? "none" : "add");
      renderMode();
    };

    modeExportBtn.onclick = () => {
      const m = getMode();
      setMode(m === "export" ? "none" : "export");
      renderMode();
    };
    ...
    // Export tools (no destructive actions)
    exportDraftBtn.onclick = async () => {
      const d = loadDraft();
      const txt = d?.snippet || "";
      if (!txt) return;
      await writeClipboard(txt);
    ...
    // This will be bound later via ui.setExportContext(novelKey)
    let exportCtx = { novelKey: null };
    exportTermMemBtn.onclick = async () => {
      if (!exportCtx.novelKey) return;
      const mem = loadTermMemory(exportCtx.novelKey);
      const txt = JSON.stringify(mem || {}, null, 2);
      await writeClipboard(txt);
    };
    ...
    return {
      ...
      setChanged: (val) => { changedTotal = clamp(Number(val) || 0, 0, 999999); refreshSummary(); },
      setMinimized: (min) => {
        const want = !!min;
      ...
      setDraftUI: (snippet, count) => setDraftUI(snippet, count),
      showGenderPicker,
      setExportContext: (ctx) => { exportCtx = Object.assign(exportCtx, ctx || {}); }
    };
  }
  ...
  // ==========================================================
  // Phase 4: Mobile long-press Add Character (Male/Female only)
  // ==========================================================
  function installLongPressAddCharacter(ui) {
    let timer = null;
    let started = false;
    ...
    document.addEventListener("pointerdown", (e) => {
      ...
      timer = setTimeout(() => {
        ...
        ui.showGenderPicker(name, (gender) => {
          const d = loadDraft();
          const items = Array.isArray(d.items) ? d.items : [];
          const line = oneLineCharacterSnippet(name, gender);
  ...
  (async () => {
    ...
    // Long-press feature (Phase 4)
    installLongPressAddCharacter(ui);
